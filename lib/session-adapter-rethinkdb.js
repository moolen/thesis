...
var SessionRethinkAdapter = function( options ){
	...
	r.connect({
		...
		// create table
		try{
			r.db(self.db).tableCreate('sessions');
		}catch(e){
			console.log(e);
		}
 ...
 */
SessionRethinkAdapter.prototype.isAdmin = function(sessionId, adminToken, callback){
	return r.table('sessions').filter(function(s){
		return s('id').eq(sessionId||null) && s('adminToken').eq(adminToken||null);
	}).run(this.conn).then(function(c){
		return c.next();
	});
};
